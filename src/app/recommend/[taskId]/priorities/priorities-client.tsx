...
function orderedFactorInfo(initialOrder: Factor[]): typeof FACTOR_INFO {
  const byFactor = new Map(FACTOR_INFO.map((item) => [item.factor, item]))
  const ordered = initialOrder
    .map((factor) => byFactor.get(factor))
    .filter((item): item is (typeof FACTOR_INFO)[number] => Boolean(item))
  const seen = new Set(ordered.map((item) => item.factor))
  return [...ordered, ...FACTOR_INFO.filter((item) => !seen.has(item.factor))]
}
...
export function PrioritiesClient({
  ...
  initialOrder: Factor[]
  initialExcluded?: Factor[]
}) {
  const { taskId } = useParams<{ taskId: string }>()
  const [items, setItems] = useState(() => orderedFactorInfo(initialOrder))
  const [disabled, setDisabled] = useState<Set<Factor>>(() => new Set(initialExcluded))
      ...
      <div className="flex flex-1 flex-col items-center px-4 py-12">
        <div className="w-full max-w-xl">
          ...
          <div className="flex flex-col items-center justify-center py-16 fade-in" role="status" aria-live="polite">
            <LoadingIndicator size="lg" label="Retaking your bearing..." sublabel="Rescoring models against your priorities" />
          </div>
        </div>
      </div>
    )
    ...
    <div className="flex flex-1 flex-col items-center px-4 py-12">
      <div className="w-full max-w-xl fade-in">
        ...
        <p className="mb-10 text-center text-grey-blue">
          Bearing has already chosen sensible priorities for this task. Change them here if your context is different.
        </p>
