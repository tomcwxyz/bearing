...
COMMENT ON TABLE user_bearing_preferences IS
  'Inspectable user defaults for automatic Bearing priorities. preferred_factors are explicit; learned tendencies are derived from authenticated structured experiment evidence and are not stored as hidden rules.';
...
COMMENT ON COLUMN user_bearing_preferences.learning_since IS
  'When set, preference learning ignores older experiment choices. Resetting preferences advances this boundary without deleting historical experiment data.';
